...
private[istio] object MixerClient {
  ...
  private[istio] def mkReportRequest(
    ...
    ReportRequest(
      attributeUpdate = Some[Attributes](
        Attributes(
          dictionary = (defaultWords.indices.zip(defaultWords)).toMap,

          stringAttributes = Map[Int, String](
            indexOf(defaultWords, "request.path") -> requestPath, // method in prom
            indexOf(defaultWords, "target.service") -> targetService // target in prom
          ),
          ...
          stringMapAttributes = Map[Int, StringMap](
            indexOf(defaultWords, "source.labels") ->
              StringMap(
                map = Map[Int, String](
                  indexOf(defaultWords, "app") -> sourceLabelApp // source in prom
                )
              ),
            indexOf(defaultWords, "target.labels") ->
              StringMap(
                map = Map[Int, String](
                  indexOf(defaultWords, "app") -> targetLabelApp, // service in prom
                  indexOf(defaultWords, "version") -> targetLabelVersion // version in prom
                )
              )
          ),
